...
function ProfilePage({self=true,img,username,level,rank,Bio="",Bagdes=[],solvedProblem=0,totalProblem=0,classname}){
    return (
        <Container classname={`text-white flex-col px-5 py-5 overflow-scroll h-screen w-[25vw] bg-[#1e2128] border-2 border-[#851fc7] ${classname}`}>
           ...
           {self && <div className="flex justify-center items-end mt-5 text-xl"> <IconButton text={"Edit"} icon={<Pen className="mx-1 w-5"/>}/> </div> }
            </div>
        </Container>
    )
}
